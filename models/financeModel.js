...
const { computeBalance, computeBudgetRemaining } = require('../utils/calculations');
...
async function getBalanceSummary(profileId) {
  ...
  const r = rows[0];
  const balance = computeBalance(r.confirmed_income, r.confirmed_expenses);
  return {
    balance,
...
async function getBudgetStatus(profileId, budgetId) {
  ...
  return { ...budget, spent, remaining: computeBudgetRemaining(budget.limit_amount, spent) };
}
